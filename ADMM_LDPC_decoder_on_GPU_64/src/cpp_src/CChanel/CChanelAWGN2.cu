...
__global__ void vectNoise(const int *IN, const double*A, const double *B, double *C, double SigB, int N)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;

    if (i < N)
    {
        double x  = sqrt(-2.0 * log( A[i] ));
        double y  = B[i];
        double Ph = x * sin(_2pi * y);
        double Qu = x * cos(_2pi * y);
#if 0
        C[i]     = ((double)(2 * IN[i]  ) + Ph * SigB) * (2.0f / (SigB * SigB));
        C[i+N]   = ((double)(2 * IN[i+N]) + Qu * SigB) * (2.0f / (SigB * SigB));
#else
        const double s2 = (2.0f * SigB * SigB);
        const double b1 = (double)(2 * IN[i]  ) - 1.0f + Ph * SigB;
        const double b2 = (double)(2 * IN[i+N]) - 1.0f + Qu * SigB;
...
void CChanelAWGN2::configure(double _Eb_N0) {
    rendement = (double) (_vars) / (double) (_data);
    if (es_n0) {
...
void CChanelAWGN2::generate( )
{
    ...
    Status = curandGenerateUniformDouble( generator, device_A, _data );
	CURAND_CALL(Status);

    Status = curandGenerateUniformDouble( generator, device_B, _data );
	CURAND_CALL(Status);

	int threadsPerBlock = 256;
    int blocksPerGrid   = (_data  + threadsPerBlock - 1) / threadsPerBlock;

    vectNoise<<<blocksPerGrid, threadsPerBlock>>>(d_IN, device_A, device_B, device_R, (double)SigB, _data);

    eStatus = cudaMemcpy(t_noise_data, device_R, _data * sizeof(double), cudaMemcpyDeviceToHost);
    CUDA_CALL(eStatus);
}
